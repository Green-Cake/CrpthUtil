...
import crpth.util.IResourceUser
...
class SoundManager {
    ...
    fun load(user: IResourceUser, name: String, path: String, doLoop: Boolean) = load(user.resourceManager, "${user.resourceUserId}:$name", path, doLoop)

    fun load(rm: ResourceManager, name: String, path: String, doLoop: Boolean): String {

        if(path.endsWith(".ogg")) {

            oggManager.loadOgg(rm, name, path, doLoop)
